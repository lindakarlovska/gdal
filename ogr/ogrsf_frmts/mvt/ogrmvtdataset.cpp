    ...
    const OGREnvelope &sEnvelope) const
{
    ...
    // Encode geometry to PBF feature
    GeometryEncoder oGeomEncoder(dfTileDim, dfTopX, dfTopY, m_nExtent);
    bool bGeomOK = oGeomEncoder.Encode(poIntersection, poGeomToEncode,
                                       poGPBFeature.get(), dfAreaOrLength);
    if (!bGeomOK)
        return OGRERR_NONE;
